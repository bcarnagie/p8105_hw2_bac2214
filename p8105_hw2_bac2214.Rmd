...
```
I imported, cleaned and tidied the dataset in such a way that all time_points are in one column instead of being split into separate columsn for baseline,time 2, etc. I also skippied the first row because this contained information about the dataset. There are `r mci_amyloid_rowtotal` measurements across all study participants and `r na_count_mciamyloid` NA values for the measurements variable.
